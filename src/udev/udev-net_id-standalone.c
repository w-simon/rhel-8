...
/* Taken from upstream commit f4dad55b8b2861426a7b9e7104dec33acb770894. */
...
static int get_virtfn_info(sd_device *dev, struct netnames *names, struct virtfn_info *ret) {
        ...
        FOREACH_DIRENT_ALL(dent, dir, break) {
                ...
                virtfn_link_file = path_join(physfn_pci_syspath, dent->d_name, NULL);
                if (!virtfn_link_file)
                        return -ENOMEM;
...
static int builtin_net_id(sd_device *dev, int argc, char *argv[], bool test) {
        ...
        if (sd_device_get_devtype(dev, &devtype) >= 0 && devtype != NULL) {
...
int main(int argc, char **argv) {
        ...
        r = sd_hwdb_new(&hwdb);
        if (r < 0) {
                fprintf(stderr, "can't initiate hwdb\n");
                return EXIT_FAILURE;
        }

        r = sd_device_new_from_syspath(&d, argv[1]);
        if (r < 0) {
